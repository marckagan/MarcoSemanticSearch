...
1. **[Single-episode search](docs/PROPOSAL.md#12-single-episode-and-cross-episode-on-device-search)** — "find the part where they talked about X," always available.
2. **[Cross-episode search](docs/PROPOSAL.md#12-single-episode-and-cross-episode-on-device-search)** — across whatever's downloaded or recently finished on-device.
3. **[Show notes as a second index](docs/PROPOSAL.md#3-show-notes-as-a-second-complementary-index)** — often higher-signal than the transcript, indexed through the same pipeline.
4. **[30-day searchable backlog](docs/PROPOSAL.md#4-30-day-searchable-backlog-for-listened-episodes)** — keeps an episode searchable for a month after finishing it, even after the audio's deleted.
5. **["What was that podcast about X?" via Siri](docs/PROPOSAL.md#5-siri--app-intents-integration--what-was-that-podcast-about-x)** — App Intents + Assistant Schemas, fully on-device, no network round-trip.
6. **[Show/host/date metadata in the index](docs/PROPOSAL.md#6-showhostdate-metadata-in-the-index)** — so a search hit resolves to a real answer ("Show Name, hosted by X, aired on Y"), not just a timestamp.
