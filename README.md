...
**Note on the tests:** they were written and confirmed to compile cleanly, but couldn't be run end-to-end in the sandboxed environment they were authored in (no full Xcode installed, so neither XCTest nor the Swift Testing runtime were available to execute them) — see each sample directory's own README for exactly what's covered and what still needs a real run before being trusted.
