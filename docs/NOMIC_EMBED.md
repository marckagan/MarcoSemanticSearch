...
## Does the model need to live on every iOS device? Disk size, RAM, and delivery

**Yes — a single copy of the Core ML-converted model must be resident on-device**, since query embedding happens fully on-device by design (no server round-trip per search — see PROPOSAL.md's opening constraint). It's not per-episode or per-search: one shared model asset, present once, reused for every query indefinitely until the app ships a model update.

**Disk size:** the baseline is ~274MB at fp16 (137M params, per the model card). That doesn't have to be the shipped size — Core ML's compression tooling supports **palettization** (weight lookup tables at 1/2/3/4/6/8-bit precision) and linear quantization. 8-bit quantization is roughly a 4x reduction over fp32 with modest accuracy impact; 4-bit palettization is documented as a common "sweet spot" for size vs. accuracy on comparable model sizes. For a model this size, a compressed Core ML build in the **~35–70MB range** is a realistic target — worth validating empirically (this design already tolerates fp16 rounding fine for retrieval, per the tokenizer-parity note above; quantization drift is generally the more forgiving axis, same logic).

**RAM usage while running:** disk size and runtime memory are related but not identical, and the relationship depends on *which* compute unit actually runs the model — a real nuance worth being precise about:

- As a rough baseline, expect runtime memory to be **in the same ballpark as the on-disk weight size**, plus modest overhead for activation/scratch buffers during the forward pass (the intermediate tensors a transformer layer computes and discards as it processes the input).
- **Crucially, this is a single-pass encoder, not a generative decoder model** — it embeds one short text and returns one fixed-size vector. There's no KV cache that grows with conversation length or context window the way a chat-style LLM has; the "memory grows with context" concern that dominates local-LLM RAM discussions doesn't apply here. Peak memory for one query embedding is close to flat, bounded by the model's own size, not by how much text has been processed over time.
- **Palettization's RAM benefit is compute-unit-dependent, and this cuts in favor of the recommended client runtime:** Apple's own guidance is that weight palettization "typically works the best on the Neural Engine for runtime memory and latency gains," with GPU execution sometimes realizing smaller or less certain memory benefits from the same compressed weights — the decompression strategy varies per hardware and needs empirical validation per model. Since Option B already recommends `.cpuAndNeuralEngine` for the client specifically (see "Neural Engine vs. GPU" above), this is a case where the runtime choice already made for other reasons (power, latency) also happens to be the one most likely to actually realize palettization's RAM savings at runtime, not just its disk savings.

**Does it need to stay in memory at all times? No.** Apple's own Core ML guidance is explicit: don't load a model at app launch on the UI thread, load it lazily (only when about to be used), keep it loaded only while actively running a sequence of predictions, and unload it under memory pressure or extended inactivity — a reload later benefits from Core ML's on-disk compiled-model cache, so it's cheaper than the very first load. This maps directly onto how this design actually uses the model: **only at the moment of an active search query**, not continuously in the background. A sensible pattern is load-on-first-search, keep resident for a short idle window in case the user searches again, then release. This also matters for staying comfortably inside iOS's per-app memory budget (Jetsam) — which is itself device-dependent and can be surprisingly tight on older/lower-RAM hardware (roughly 350MB on the oldest supported devices vs. ~4GB on current flagships); a permanently-resident ~35–70MB (compressed) or ~274MB (uncompressed) model competing against everything else the app holds in memory (audio buffers, UI, cached artwork) is a much easier budget to hit with lazy loading than with an always-loaded singleton.

**Delivery — bundle it, or fetch it after install?** Two real options, with a timing-relevant gotcha:

- **Bundle it directly in the app binary.** Simplest — zero extra engineering, model is always present. Cost: every install pays the full model size, even users who never use search.
- **Fetch it after install**, so the initial download/install stays small and the model downloads once in the background (first launch, or opportunistically on Wi-Fi/idle). The traditional mechanism for this — **On-Demand Resources** — is a poor choice for a project targeting iOS 27 specifically: **ODR is deprecated as of iOS 27/iPadOS 27/watchOS 27/tvOS 27**, with Apple explicitly steering developers toward the newer **Background Assets** framework instead (`BADownloadManager`, Apple-Hosted Background Assets includes 200GB per app in an Apple Developer Program membership). Given this design already targets iOS 27, **Background Assets is the right mechanism**, not ODR — building on a framework Apple is actively sunsetting on day one of the target OS would be a bad trade for a small app-size win.

**Practical implication:** search shouldn't be assumed available the instant the app launches for the first time if the model is fetched via Background Assets rather than bundled — there's a one-time background download (tens of MB, post-compression) before on-device query embedding works. The chunk *data* (text + pre-computed embeddings from the farm) can sync and populate SQLite immediately regardless — only the client's own query-embedding capability is gated on the model being present. Worth an explicit "search index still downloading" state in the UI rather than assuming instant availability.

Sources: [Compressing Neural Network Weights — Guide to Core ML Tools](https://apple.github.io/coremltools/docs-guides/source/quantization-neural-network.html), [Overview — Guide to Core ML Tools](https://apple.github.io/coremltools/docs-guides/source/opt-overview.html) (palettization's compute-unit-dependent runtime memory behavior), [On-demand resources size limit — App Store Connect Help](https://developer.apple.com/help/app-store-connect/reference/app-uploads/on-demand-resources-size-limits/) (deprecation notice), [Meet Background Assets — WWDC22](https://developer.apple.com/videos/play/wwdc2022/110403/), [Migrate On-Demand Resources To Localized Managed Background Assets — The Swift Dev](https://www.theswift.dev/posts/migrate-on-demand-resources-to-localized-managed-background-assets/), [How to Build AI iOS Apps: Complete CoreML Guide — Medium](https://medium.com/@iniyarajan/how-to-build-ai-ios-apps-complete-coreml-guide-63ee4632b413) (lazy-load/unload guidance), [Jetsam memory limits by device — home-assistant/iOS issue #4475](https://github.com/home-assistant/iOS/issues/4475) and [Memory Efficiency in iOS: Metrics](https://antongubarenko.substack.com/p/memory-efficiency-in-ios-metrics) (per-device memory budget figures)
