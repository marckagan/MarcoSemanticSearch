...
Two new tables, no new extension. `source_type` and nullable timestamps accommodate show-notes and episode-identity chunks alongside transcript chunks (see [FEATURE_ROADMAP.md](FEATURE_ROADMAP.md)); `episodes` is not new storage — it's whatever table already backs the app's library UI, shown here only to make the join explicit:
...
### GPU, yes. Neural Engine — not for the scan step itself, and here's why

Worth being precise about which of iOS's two accelerators (GPU, ANE) each part of the on-device query path actually can and should target, since they're not interchangeable the way "use faster hardware" might suggest:

- **GPU: yes — that's exactly Tier 1 above.** A brute-force cosine scan is a matrix-vector multiply, which is squarely what GPUs are built for and what `Metal`/`MPSMatrix` expose directly. No caveat here beyond the one already noted: worth reaching for once profiling shows the CPU path is actually too slow, not as a default starting point — a GPU command-buffer dispatch has its own fixed overhead (kernel setup, synchronization), and at the small-to-moderate chunk counts this design's own retention rules actually produce (per the numbers above, realistically under ~10,000 chunks even for a heavy user), that fixed overhead can easily exceed the entire CPU `vDSP` computation time. GPU brute force is a scaling lever to pull later, not a reason to add Metal complexity now.
- **Neural Engine: no, not directly — and not really indirectly either.** This is a hard constraint, not a design preference: **Apple provides no public API to program the ANE directly at all.** The only way to reach it is through Core ML, which compiles a defined model graph and lets Core ML's own scheduler decide, per-op, whether to run each part on ANE, GPU, or CPU — there's no way to hand it an ad hoc "run this matrix multiply" instruction outside that model-graph abstraction, and `Metal` explicitly cannot target the ANE at all (it's GPU-only). Technically, one *could* try to disguise the cosine scan as a tiny Core ML "model" to bait the scheduler into using the ANE, but this fights the hardware's actual design: the chunk count changes every query (a dynamic shape), while the ANE strongly prefers fixed, precompiled shapes for its instruction pipelining; a Core ML prediction call carries more fixed dispatch/compilation overhead than a Metal compute dispatch, which is the wrong direction for something invoked fresh on every search; and the ANE's whole design target is deep, mostly-fixed neural-network graphs (convolutions, attention layers), not ad hoc linear algebra over a table whose size changes constantly. There's no real upside over just using Metal, and real friction in getting there.
- **Where the ANE does fit — already in this design:** the *embedding* step, not the scan. Turning the query text into a vector is exactly the fixed-shape, deep neural-network forward pass the ANE is built for — which is why Option B already targets Core ML with `.cpuAndNeuralEngine` for that step (see "Neural Engine vs. GPU" above). The scan that happens *after* the vector exists is a different category of computation — data-parallel linear algebra over a dynamically-sized table — and that's GPU territory, not ANE territory, for architectural reasons rather than an arbitrary implementation choice.

Sources: [neural-engine/docs/ane-vs-gpu.md — hollance](https://github.com/hollance/neural-engine/blob/master/docs/ane-vs-gpu.md) ("There is currently no public framework for directly programming the ANE... developers must use Core ML"), [Evaluation of Domain-Specific Architectures for General-Purpose Applications in Apple Silicon (arXiv)](https://arxiv.org/pdf/2511.13450) ("Metal cannot be used to program the ANE, it's exclusively for the GPU")
...
Six extensions worth designing for now beyond the core mechanism above — single/cross-episode search, show notes indexing, a 30-day searchable backlog for listened episodes, Siri/App Intents integration, and show/host/date metadata in the index. Moved to its own page, [FEATURE_ROADMAP.md](FEATURE_ROADMAP.md), since it grew large enough to read independently of the core architecture.
...
```mermaid
pie showData
    title "Pieces of the design, by category"
    "Marco's own code" : 9
    "Apple (system framework, proprietary)" : 2
    "Apple (open source)" : 2
    "Non-Apple OSS" : 3
    "Public domain" : 1
```

**Owned code, split by where it runs:**

#### Server side (Mac Mini farm)

| Piece | What it does | Owned by Marco? |
|---|---|---|
| Chunker (sentence/time-windowed segmentation) | Splits a transcript into timestamped chunks | **Yes — his code** |
| Embedder wrapper (prefix handling, calls into MLX) | Wraps `nomic-embed-text-v1.5` via `mlx-swift`, applies `"search_document: "` | **Yes — his code** |
| Payload builder (chunks → sync payload JSON) | Assembles chunks + embeddings into the sync payload | **Yes — his code** |
| Sync/serve logic (cursor-based diffing) | Tracks what's changed per episode, serves incremental updates | **Yes — his code** |

#### Client side (iOS)

| Piece | What it does | Owned by Marco? |
|---|---|---|
| Sync/pull logic (incremental apply) | Requests and applies chunks changed since the client's last cursor | **Yes — his code** |
| SQLite schema (`transcript_chunks`, `episodes` join, FTS5 virtual table) | Defines how chunks and vectors are stored alongside existing data | **Yes — his schema** |
| Storage layer (insert/upsert/prune, `SearchIndexStore`) | Writes synced chunks, enforces the 30-day retention prune | **Yes — his code** |
| Query engine (brute-force cosine ranking, `QueryEngine`) | Embeds the query (via Core ML) and ranks chunks by similarity | **Yes — his code** |
| App Intents (`FindEpisodeByTopicIntent`, episode `AppEntity`, dialog assembly) | The Siri-facing entry point and response assembly | **Yes — his code** |

**The short version: every line of logic that actually implements this feature — on both the farm and the client — is code Marco writes and owns outright.** The third-party surface below is entirely underlying *infrastructure* (a tokenizer, a model's weights, a math library, an inference runtime) that any on-device ML feature would need from somewhere — none of it is the feature itself, and none of it is behind a paywall or commercial license.

**Third-party dependencies, by category:**

| Component | Role | Category | License | Shipped at runtime? |
|---|---|---|---|---|
| SQLite + FTS5 | Storage, keyword search | Public domain | Public domain | Yes, client (already true today, unchanged) |
| Accelerate / vDSP | Cosine similarity math | Apple (system framework, proprietary) | Proprietary, free | Yes, client |
| Core ML | ANE-eligible inference (Option B/C) | Apple (system framework, proprietary) | Proprietary, free | Yes, client |
| MLX / MLX Swift | GPU inference, model loading | Apple (open source) | MIT | Yes, farm (both options); client only under Option A |
| `coremltools` (Apple) | HF → Core ML weight conversion (Option B/C) | Apple (open source) | BSD-3-Clause | No — build/conversion-time only |
| `swift-transformers` (Hugging Face) | WordPiece tokenizer, client side | Non-Apple OSS | Apache-2.0 | Yes, client |
| `nomic-embed-text-v1.5` weights | The embedding model itself | Non-Apple OSS | Apache-2.0 | Yes, both sides (as a bundled model file, not code) |
| `mlx-embeddings` (community, Prince Canuma) | HF → MLX weight conversion | Non-Apple OSS | **GPLv3** | No — build/conversion-time only, not linked into anything shipped |

Worth noting the split isn't quite "Apple vs. everyone else" — it's finer than that: two of the Apple pieces are closed system frameworks that simply ship with the OS (`Accelerate`, `Core ML`), while two others are Apple-authored but genuinely open source (`MLX`/`MLX Swift` is Apple's `ml-explore` org, MIT-licensed; `coremltools` is Apple's own conversion tooling, BSD-3). The non-Apple OSS category is where the one licensing wrinkle lives — `mlx-embeddings`, flagged below.
...
See [server-sample/](../server-sample) and [client-sample/](../client-sample) for illustrative (not production-ready) code sketches of the chunk+embed step and the client store/query engine — each is a small SwiftPM package with unit tests (23 total, all passing) for the logic that doesn't depend on a real MLX/Core ML model (chunking, cosine-ranking, the SQLite round-trip, both prefix-correctness checks); see each directory's README for exactly what's covered and how to run them. Also [NOMIC_EMBED.md](NOMIC_EMBED.md) for the embedding model deep dive, [PLATFORM_COMPATIBILITY.md](PLATFORM_COMPATIBILITY.md) for iOS 27 opportunities and minimum-iOS-version compatibility, and [FEATURE_ROADMAP.md](FEATURE_ROADMAP.md) for planned extensions beyond the core mechanism.
