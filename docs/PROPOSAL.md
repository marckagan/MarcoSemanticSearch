...
See [server-sample/](../server-sample) and [client-sample/](../client-sample) for illustrative (not production-ready) code sketches of the chunk+embed step and the client store/query engine — each is a small SwiftPM package with `swift test`-runnable unit tests for the logic that doesn't depend on a real MLX/Core ML model (chunking, cosine-ranking, the SQLite round-trip, both prefix-correctness checks); see each directory's README for exactly what's covered. Also [NOMIC_EMBED.md](NOMIC_EMBED.md) for the embedding model deep dive, [PLATFORM_COMPATIBILITY.md](PLATFORM_COMPATIBILITY.md) for iOS 27 opportunities and minimum-iOS-version compatibility, and [FEATURE_ROADMAP.md](FEATURE_ROADMAP.md) for planned extensions beyond the core mechanism.
